...
class TableHyperImg(HyperImg):
    ...
    def __init__(self, path: str, table: pd.DataFrame,
        ...
        super().__init__(path, threshold_value, savgol_par, target_varible_name, with_median=with_median)
    ...
    def _get_tiff(self) -> np.ndarray:
        ...
        self.plant_number = self.table[self.table[self.name_column] == self.path.split('/')[-1]][self.plant_column].iloc[0]
