...
from hyper_img.hyperImg_functions.get_data_funcs import get_mean_diff_and_95_confident_interval_df, \
...
def get_mean_diff_and_95_confident_interval_graph(hyper_imges: tp.Sequence[HyperImg],
                                                  target_variable_1: str,
                                                  target_variable_2: str,
                                                  download_path: str = '') -> None:
    """
    Plots the 95% confidence interval for sample mean differences.
    ...
    m, l, r = get_mean_diff_and_95_confident_interval_df(hyper_imges, target_variable_1, target_variable_2)
    ...
    fig.update_layout(title=f'95% confident interval for difference between sample means '
...
def get_medians_wavelenght_graph(hyper_imges: tp.Sequence[HyperImg],
    ...
    if color is not None:
        fig = px.line(df, x='Wavelength', y='Median', color=hyper_imges[0].target_varible_name,
                      color_discrete_map=color, line_group=df['Sample'], **kwargs)
    else:
        fig = px.line(df, x='Wavelength', y='Median', color=hyper_imges[0].target_varible_name,
...
def get_2_pca_graph(hyper_imges: tp.Sequence[HyperImg],
    ...
    if color is not None:
        fig = px.scatter(df, x='1', y='2', color=hyper_imges[0].target_varible_name, color_discrete_map=color)
    else:
        fig = px.scatter(df, x='1', y='2', color=hyper_imges[0].target_varible_name)
...
def get_umap_graph(hyper_imges: tp.Sequence[HyperImg],
    ...
    if color is not None:
        fig = px.scatter(df, x=X[:, 0], y=X[:, 1], color=hyper_imges[0].target_varible_name, color_discrete_map=color)
    else:
        fig = px.scatter(df, x=X[:, 0], y=X[:, 1], color=hyper_imges[0].target_varible_name)
