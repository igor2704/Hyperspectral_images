...
def get_df_graphics_medians_wavelenght(hyper_imges: tp.Sequence[HyperImg]) -> pd.DataFrame:
    ...
    for sample_number, sample in enumerate(hyper_imges):
        for p in zip(x_axis, sample.medians):
            points.append([p[0], p[1], sample_number, sample.target_variable, sample.plant_number])
...
def get_df_medians(hyper_imges: tp.Sequence[HyperImg], 
    ...
    return pd.DataFrame([list(sample.medians) + [sample.target_variable, sample.plant_number] for sample in hyper_imges
...
def get_df_pca_and_explained_variance(hyper_imges: tp.Sequence[HyperImg],
    ...
    lst_of_value = [list(new_arr[i][0][:]) + [new_arr[i][1]] + [df['PlantNumber'][i]] for i, _ in enumerate(new_arr)]
    columns_numbers = [str(i + 1) for i in range(n_components)]
    return pd.DataFrame(lst_of_value,
                        columns=columns_numbers + [hyper_imges[0].target_varible_name, 'PlantNumber']), \
...
def get_df_umap(hyper_imges: tp.Sequence[HyperImg],
    ...
    X = pipe.fit_transform(X)
    manifold = umap.UMAP(n_components=n_components, **kwargs_sklearn_umap).fit(X, y)
...
def get_df_em_algorithm_clustering(hyper_imges: tp.Sequence[HyperImg],
    ...
    if downscaling_method == 'PCA':
        ...
        df = get_df_isomap(filtred_hyper_imges, n_components=dim_clusterization)
    elif downscaling_method == 'UMAP':
        df = get_df_umap(filtred_hyper_imges, n_components=dim_clusterization)
    else:
